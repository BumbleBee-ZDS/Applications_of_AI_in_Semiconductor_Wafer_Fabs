"""
端到端测试脚本 (修复版): 跳过已生成数据, 修复 is_configured 属性和 What-If 列名
"""
...
try:
    ...
    api_configured = client.is_configured  # property, not method
...
# =============================================================================
# Step 2: 数据已生成 (跳过, 仅记录统计)
# =============================================================================
print("=" * 70)
print("Step 2: 数据统计 (LLM 润色数据已在上一步生成)")
print("=" * 70)
...
print(f"  equipment: {gen_stats['equipment']}")
print(f"  lots: {gen_stats['lots']}")
print(f"  lot_history: {gen_stats['lot_history']}")
print(f"  equipment_events: {gen_stats['equipment_events']}")
print(f"  daily_output: {gen_stats['daily_output']}")
results["data_gen"] = {"stats": gen_stats, "llm_polish": True}
print()
...
for tbl, desc, min_expected in table_specs:
    ...
    ok = cnt >= min_expected
    status = "✅" if ok else "⚠️"
    print(f"  {status} {tbl}: {cnt} 行 (预期≥{min_expected})")
...
for cls in test_classes:
    ...
    for method_name in methods:
        ...
        try:
            ...
            print(f"  ✗ {cls.__name__}::{method_name}  ({dur:.0f}ms)  {err_msg}")
...
try:
    ...
    for c in report.causes[:5]:
        if hasattr(c, "__dict__"): c = vars(c)
    ...
    for s in report.suggestions[:5]:
        if hasattr(s, "__dict__"): s = vars(s)
...
try:
    ...
    results["predictor"] = {
        "local": {"time_ms": round(local_time, 1), "method": fr_local.method, "mape": fr_local.mape, "predicted": [int(v) for v in fr_local.predicted]},
        "llm": {"time_ms": round(llm_time, 1), "method": fr_llm.method, "mape": fr_llm.mape, "predicted": [int(v) for v in fr_llm.predicted], "used_llm": fr_llm.used_llm},
    }
    print(f"  ✅ 预测验证完成")
except Exception as e:
    print(f"  ❌ 预测失败: {e}")
...
try:
    ...
    for _, row in df_compare.iterrows():
        # 修正列名: total_wafers_per_week / delta_wafers
        ...
        print(f"    {row.get('name', '')}: {weekly:,} 片 ({delta_str}, {safe_round(delta_p*100,2)}%)")
    results["what_if"] = {
...
try:
    ...
    llm = get_llm()
    llm_ok = llm.is_configured  # property, not method
    if llm_ok:
