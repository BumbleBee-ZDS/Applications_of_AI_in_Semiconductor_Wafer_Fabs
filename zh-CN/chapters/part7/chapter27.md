...
本章收录 22 个动手实验，全部来自作者在真实开发中积累并验证过的 MVP 项目，按主题与正文章节的对应关系组织：
...
| 27.24 FabWiki 数据字典与血缘维基 | 知识图谱 + Text2SQL + 隐性知识显式化 | 第28章 | ★★☆ |
...
**学什么**：这是 22 个实验中工程化程度最高的项目，演示晶圆厂数据资产的"元数据治理 + 语义检索"：
...
**学什么**：22 个实验中文档、测试与降级策略最完整的项目，演示"感知（Perception）-思考（Thinking）-行动（Action）"四 Agent 编排的产能分析：
...
对应**第9章（良率爬坡）**与**第11章（建设期/爬坡期）**。纯 Python 实现三大概念：Poisson / 负二项式 / Murphy 良率模型对比、S 形爬坡曲线与学习率、基于 FDC 信号的虚拟量测入门预测。运行后生成三张图并输出模型对比结论，直观理解 `Y = exp(-D₀A)` 的数学含义与"死亡之谷"的成因。代码与说明均为中英双语。
...
对应**第12章（成熟量产期·预测性维护）**。用合成设备退化数据实现剩余寿命（RUL）预测，并对比"定期 PM"与"预测性维护"两种策略的维护成本——回答"维护时机到底怎么定"这一成熟期核心问题。代码与说明均为中英双语。
...
对应**第22章（LLM在晶圆厂的应用）**。实现一个精简 RAG 系统：从工艺规范（SPEC）文档库检索相关内容，再调用 LLM 生成带来源引用的回答。默认调用 DeepSeek API；未配置 API Key 时自动降级为 Mock LLM，可离线运行。代码与说明均为中英双语。

```bash
cd experiments/llm_rag_spec_qa
pip install requests
echo "DEEPSEEK_API_KEY=你的key" > .env   # 可选，不配置则用 Mock LLM
...
## 27.21 实验十九：多 Agent 协作诊断（multi_agent_collaboration）

对应**第23章（多 Agent 系统架构）**。模拟晶圆厂多 Agent 协作：主持人 Agent 收到质量问题后分别咨询工艺、设备、良率、调度四个专业 Agent，收集意见后汇总为最终决策——演示跨部门协同的 Agent 编排。支持 DeepSeek API 与离线 Mock。代码与说明均为中英双语。

```bash
cd experiments/multi_agent_collaboration
pip install requests matplotlib flask
echo "DEEPSEEK_API_KEY=你的key" > .env   # 可选
python multi_agent_collaboration.py  # 命令行 / CLI
python web_app.py                  # Web 界面 http://127.0.0.1:5009
```

## 27.22 实验二十：Agent 任务分解与执行（agent_task_decomposition）

对应**第17章（SA 符号行为融合）**与第23章 Agent 规划组件。实现规划-执行型 Agent：把高层目标自动分解为可执行子任务，逐一调用工具执行并汇总为完成报告——演示"符号规划做方向，行为执行做落地"。支持 DeepSeek API 与离线 Mock。代码与说明均为中英双语。

```bash
cd experiments/agent_task_decomposition
pip install requests matplotlib flask
echo "DEEPSEEK_API_KEY=你的key" > .env   # 可选
python agent_task_decomposition.py  # 命令行 / CLI
python web_app.py                  # Web 界面 http://127.0.0.1:5010
```

## 27.23 实验二十一：反思型 Agent（reflexion_agent）

对应**第23章（Agent 自我改进）**。实现 Reflexion 循环：Agent 尝试负载均衡派工 → 评估 → 自我反思失败原因 → 携带反思重试，逐轮改进——演示 Agent 的自我改进机制。支持 DeepSeek API 与离线 Mock。代码与说明均为中英双语。

```bash
cd experiments/reflexion_agent
pip install requests matplotlib flask
echo "DEEPSEEK_API_KEY=你的key" > .env   # 可选
python reflexion_agent.py          # 命令行 / CLI
python web_app.py                  # Web 界面 http://127.0.0.1:5011
```

## 27.24 实验二十二：FabWiki 数据字典与血缘维基（fabwiki）

对应**第28章（晶圆厂数据体系）**。这是与数据治理主题最匹配的配套实验：把 Oracle 数仓中散落在存储过程与老工程师脑中的**隐性知识**（魔法编码、前缀编码、字段类型陷阱、口径时序陷阱）显式沉淀为 Markdown 知识包（OKF 格式），编译成可导航的知识图谱，让 LLM **确定性读取**知识后生成 Text2SQL——演示"数据字典做不到的、知识图谱能做到的"。

```bash
cd experiments/fabwiki
pip install -r requirements.txt
streamlit run app.py        # 4个页面: 首页/知识浏览/血缘图谱/Text2SQL
```

**学什么**：8 张 Fab 风格表 + 5 类隐性知识陷阱的建模方式；"定位 → 确定读全文 → 生成 → 执行 → 失败重试"的知识导航式 Text2SQL；离线（无 API Key）也能完整演示。

**怎么跑**：启动后进入 Text2SQL 页面提问，如"查询昨天良率低于 80% 的批次"，对比开启知识导航前后的 SQL 质量差异；再到血缘图谱页查看字段间的派生关系。

**看什么**：同样的自然语言问题，朴素 Text2SQL 会写出错误 SQL（如漏掉 `LOT_STS<>'99'` 的测试批次过滤），知识导航版则能正确引用隐性知识——这正是第28章"隐性知识显式化"思想的直观演示。
