...
- 26.1 从"会思考"到"会动手"
  - 具身智能（Embodied AI）定义与晶圆厂的强需求
  - 与第21章NSA全融合的关系：智能闭环在物理世界的落地形态
- 26.2 具身智能的技术栈
  - 多模态感知、世界模型、认知规划、行动执行、学习与反馈
  - VLA（视觉-语言-行动）模型
- 26.3 晶圆厂中的具身智能场景
  - AMHS天车与AGV智能搬运
  - 洁净室机械臂与EFEM自动上下料
  - 移动操作机器人巡检与维护
  - 人形机器人（探索阶段）
- 26.4 具身智能与Ontology的融合
  - Ontology作为世界模型：可验证性与可编排性
- 26.5 实践现状与案例
- 26.6 挑战与展望
  - 技术、成本、组织挑战与演进路径
- 26.7 本章小结
...
- 28.1 晶圆厂数据全景：从MES到设备日志
  - 28.1.1 核心数据源地图：MES / FDC-EDA / SPC / YMS / ERP / SECS-GEM / 检测量测
  - 28.1.2 数据特征对比：时序粒度、量级与更新频率（FDC毫秒级 vs MES分钟级 vs YMS批次级）
  - 28.1.3 数据之间的业务关联：跨系统关联的必要性与主键/时间对齐/多版本难点
- 28.2 数据血缘与隐性知识：数据从哪来、到哪去、谁懂它
  - 28.2.1 晶圆厂数据血缘的复杂性来源（存储过程/PL-SQL/ETL七层加工链）
  - 28.2.2 隐性知识的三种形态：代码中的 / 文档中的 / 人脑中的
  - 28.2.3 隐性知识流失的业务风险
- 28.3 数据治理体系：让数据可信、可追溯、可复用
  - 28.3.1 数据质量挑战：缺失值 / 异常值 / 口径不一致 / 时序对齐
  - 28.3.2 数据标准化与元数据管理（技术/业务/操作三类元数据）
  - 28.3.3 数据血缘自动采集的技术路径（静态解析 / 运行时审计 / 人工补录）
  - 28.3.4 血缘的图存储方案（关系型 vs 图数据库）
- 28.4 工业语义层与知识图谱：让AI懂晶圆厂的语言
  - 28.4.1 为什么互联网数据中台模式不能直接照搬
  - 28.4.2 工业语义层的概念与定位
  - 28.4.3 半导体领域本体模型（Ontology）设计：六大核心实体与关系
  - 28.4.4 知识图谱的分层架构：L1血缘 → L2业务知识 → L3可操作本体
  - 28.4.5 与业界方案的对比（Google OKF / Palantir Ontology）
- 28.5 LLM与Agent的数据消费模式：从"查数据"到"用数据决策"
  - 28.5.1 GraphRAG vs 传统向量RAG
  - 28.5.2 多Agent协作架构设计（感知/诊断/知识问答/执行）
  - 28.5.3 数据权限与安全控制
- 28.6 落地路径与分阶段实施：从数据到价值的路线图
  - 28.6.1 四阶段路线图总览
  - 28.6.2 各阶段关键里程碑与验收标准
  - 28.6.3 常见陷阱与避坑指南
- 28.7 配套实验：FabWiki 数据字典与血缘维基（fabwiki，第27章 27.24 节）
