...
- 28.1 晶圓廠數據全景：從MES到設備日誌
  - 28.1.1 核心數據源地圖：MES / FDC-EDA / SPC / YMS / ERP / SECS-GEM / 檢測量測
  - 28.1.2 數據特徵對比：時序粒度、量級與更新頻率（FDC毫秒級 vs MES分鐘級 vs YMS批次級）
  - 28.1.3 數據之間的業務關聯：跨系統關聯的必要性與主鍵/時間對齊/多版本難點
- 28.2 數據血緣與隱性知識：數據從哪來、到哪去、誰懂它
  - 28.2.1 晶圓廠數據血緣的複雜性來源（儲存程序/PL-SQL/ETL七層加工鏈）
  - 28.2.2 隱性知識的三種形態：程式中的 / 文件中的 / 人腦中的
  - 28.2.3 隱性知識流失的業務風險
- 28.3 數據治理體系：讓數據可信、可追溯、可複用
  - 28.3.1 數據品質挑戰：缺失值 / 異常值 / 口徑不一致 / 時序對齊
  - 28.3.2 數據標準化與元數據管理（技術/業務/操作三類元數據）
  - 28.3.3 數據血緣自動採集的技術路徑（靜態解析 / 執行期稽核 / 人工補錄）
  - 28.3.4 血緣的圖儲存方案（關聯式 vs 圖資料庫）
- 28.4 工業語義層與知識圖譜：讓AI懂晶圓廠的語言
  - 28.4.1 為什麼網際網路數據中台模式不能直接照搬
  - 28.4.2 工業語義層的概念與定位
  - 28.4.3 半導體領域本體模型（Ontology）設計：六大核心實體與關係
  - 28.4.4 知識圖譜的分層架構：L1血緣 → L2業務知識 → L3可操作本體
  - 28.4.5 與業界方案的對比（Google OKF / Palantir Ontology）
- 28.5 LLM與Agent的數據消費模式：從「查數據」到「用數據決策」
  - 28.5.1 GraphRAG vs 傳統向量RAG
  - 28.5.2 多Agent協作架構設計（感知/診斷/知識問答/執行）
  - 28.5.3 數據權限與安全控制
- 28.6 落地路徑與分階段實施：從數據到價值的路線圖
  - 28.6.1 四階段路線圖總覽
  - 28.6.2 各階段關鍵里程碑與驗收標準
  - 28.6.3 常見陷阱與避坑指南
- 28.7 配套實驗：FabWiki 數據字典與血緣維基（fabwiki，第27章 27.24 節）
