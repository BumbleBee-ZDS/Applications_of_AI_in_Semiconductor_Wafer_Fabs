...
對於複雜產品，晶片良率還受到電路密度和關鍵面積（Critical Area）的影響——並非晶片上所有區域都對缺陷同樣敏感，只有落在關鍵區域（如金屬線間距最小處）的缺陷才會導致失效[81][82]。關鍵面積越小，晶片對缺陷的容忍度越高。
...
> **本章配套實驗**：第27章 27.12 節的良率模型與爬坡模擬實驗（`demos/experiments/yield_modeling_ramp`）用程式重現了 Poisson/負二項式良率模型與 S 形爬坡曲線，並提供虛擬量測入門範例，可直接執行驗證本章結論。
