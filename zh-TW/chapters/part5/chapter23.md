...
> **本章配套實驗**：第27章 27.21 節的多 Agent 協作診斷實驗（`demos/experiments/multi_agent_collaboration`）演示主持人 Agent 編排四個專業 Agent 協同診斷；27.23 節的反思型 Agent（`demos/experiments/reflexion_agent`）演示 Agent 透過「評估+反思+重試」實現自我改進。
