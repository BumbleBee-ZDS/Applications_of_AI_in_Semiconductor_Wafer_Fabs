...
本章收錄 22 個動手實驗，全部來自作者在真實開發中積累並驗證過的 MVP 專案，按主題與正文章節的對應關係組織：
...
| 27.24 FabWiki 數據字典與血緣維基 | 知識圖譜 + Text2SQL + 隱性知識顯式化 | 第28章 | ★★☆ |

> 所有實驗程式碼位於倉庫 `experiments/` 目錄（各專案自帶 README）。除標註「需 API Key」的實驗外，其餘均可離線執行；需要 LLM 的實驗均提供 Mock/降級模式，無 Key 也能體驗核心流程。
...
**學什麼**：這是 22 個實驗中工程化程度最高的專案，演示晶圓廠資料資產的「元資料治理 + 語義檢索」：
...
**學什麼**：22 個實驗中文檔、測試與降級策略最完整的專案，演示「感知（Perception）-思考（Thinking）-行動（Action）」四 Agent 編排的產能分析：
...
## 27.24 實驗二十二：FabWiki 數據字典與血緣維基（fabwiki）

對應**第28章（晶圓廠數據體系）**。這是與數據治理主題最匹配的配套實驗：把 Oracle 資料倉儲中散落在儲存程序與老工程師腦中的**隱性知識**（魔法編碼、前綴編碼、欄位型別陷阱、口徑時序陷阱）顯式沉澱為 Markdown 知識包（OKF 格式），編譯成可導覽的知識圖譜，讓 LLM **確定性讀取**知識後生成 Text2SQL——演示「數據字典做不到的、知識圖譜能做到的」。

```bash
cd experiments/fabwiki
pip install -r requirements.txt
streamlit run app.py        # 4個頁面: 首頁/知識瀏覽/血緣圖譜/Text2SQL
```

**學什麼**：8 張 Fab 風格表 + 5 類隱性知識陷阱的建模方式；「定位 → 確定讀全文 → 生成 → 執行 → 失敗重試」的知識導覽式 Text2SQL；離線（無 API Key）也能完整演示。

**怎麼跑**：啟動後進入 Text2SQL 頁面提問，如「查詢昨天良率低於 80% 的批次」，對比開啟知識導覽前後的 SQL 品質差異；再到血緣圖譜頁查看欄位間的衍生關係。

**看什麼**：同樣的自然語言問題，樸素 Text2SQL 會寫出錯誤 SQL（如漏掉 `LOT_STS<>'99'` 的測試批次過濾），知識導覽版則能正確引用隱性知識——這正是第28章「隱性知識顯式化」思想的直觀演示。
