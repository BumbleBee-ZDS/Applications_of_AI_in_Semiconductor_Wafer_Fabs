...
本章收錄 21 個動手實驗，全部來自作者在真實開發中積累並驗證過的 MVP 專案，按主題與正文章節的對應關係組織：
...
**學什麼**：這是 21 個實驗中工程化程度最高的專案，演示晶圓廠資料資產的「元資料治理 + 語義檢索」：
...
**學什麼**：21 個實驗中文檔、測試與降級策略最完整的專案，演示「感知（Perception）-思考（Thinking）-行動（Action）」四 Agent 編排的產能分析：
